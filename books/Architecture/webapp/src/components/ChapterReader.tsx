...
export default function ChapterReader({ chapter, content }: ChapterReaderProps) {
  ...
  return (
    <div className="min-h-screen">
      ...
      {/* Top navigation */}
      <nav className="sticky top-0 z-40 h-14 backdrop-blur-md bg-surface/80 border-b border-border">
        <div className="h-full px-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <span className="text-xs text-foreground/50 mr-2 p-2">
      ...
      <div className="flex">
        {/* Sidebar */}
        <aside
          className={`fixed lg:sticky top-14 p-2 left-0 z-30 h-[calc(100vh-3.5rem)] w-72 shrink-0 bg-[var(--surface)] border-r border-[var(--border-color)] overflow-y-auto transition-transform duration-300 ${sidebarOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"
            }`}
        >
          <div className="p-4">
            ...
            {CHAPTERS.map((ch) => (
              <Link
                ...
                className={`block px-3 py-2 rounded-lg text-sm mb-1 transition-all ${ch.slug === chapter.slug
        ...
        {/* Main content */}
        <main className="flex-1 min-w-0 w-full px-4 sm:px-6 md:px-10 py-8 sm:py-10">
          <div className="max-w-3xl mx-auto">
            ...
            {/* Chapter navigation */}
            <div className="mt-16 pt-8 border-t border-[var(--border-color)] flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-4">
              {prevChapter ? (
                <Link
