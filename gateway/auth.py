...
from .utils import APIKeyError, validate_uuid
...
class APIKeyAuth:
    ...
    def generate_token(self, user_id: str, service_id: str) -> str:
        ...
        payload = {
            ...
            "jti": hashlib.sha256(f"{user_id}:{service_id}:{datetime.utcnow().timestamp()}".encode()).hexdigest()[:16]
        }
    ...
    def authenticate_api_key(self, api_key: str) -> Tuple[bool, Optional[Dict]]:
    ...
    def create_api_key(self, user_id: str, service_id: str, permissions: list) -> str:
        ...
        self.redis_client.setex(f"api_key:{api_key}", 2592000, json.dumps(key_data))  # 30 days
    ...
    def invalidate_api_key(self, api_key: str):
        ...
        if key_data:
            ...
            self.redis_client.srem(f"service_keys:{key_info.get('service_id')}", api_key)
    ...
    def get_api_key_usage_stats(self, api_key: str, hours: int = 24) -> Dict:
        ...
        stats_key = f"api_key_stats:{api_key}:{int(datetime.utcnow().timestamp() / 3600)}"
    ...
    def log_api_key_usage(self, api_key: str, user_id: str, service_id: str, status_code: int, response_time: float, source_ip: str):
