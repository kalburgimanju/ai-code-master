...
description: >
  ...
  display + Inter body + SF Mono for the index/code voice on a warm-navy code surface,
...
components:
  ...
  number-lockup:
    ...
    description: "Hero stat — a Inter figure paired with a SF Mono unit (200K, +1,204, −318, 17 files). Figure is serif; the unit is ALWAYS mono, never the serif."
  pull-quote:
...
- **Fonts** — already set to **Inter** (display) / **SF Mono** (body); ignore any preset font name lingering in prose.
...
- **Inter** (sentence case, negative-tracked) for all display; **Inter** body/chrome; **SF Mono** index + code.
...
- **Type** — Inter sentence-case display (negative-tracked); Inter 400 body; SF Mono kickers (uppercase 0.16em, coral ✱) + code.
...
- **Inter display is sentence case** (NOT title case, NOT uppercase), weight 400, negative-tracked (−0.012..−0.028em); reach for **italic** when the line is a stance or a definition. **Inter body** sentence case weight 400. **SF Mono** kickers UPPERCASE 0.16em with the coral ✱. No uppercase serif, no sans headline, no serif label.
...
- Set all display in **Inter, sentence case**, negative-tracked; **Inter 400** body; **SF Mono** kickers (uppercase, 0.16em, coral ✱) + code.
...
- **Type** — Inter sentence-case display (negative-tracked); Inter 400 body; SF Mono kickers (uppercase 0.16em, coral ✱) + code; ≥1.4cqw floor.
